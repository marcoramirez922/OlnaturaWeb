...
class Inicio extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->library('Gtranslate_lib');
	...
	public function index($content=""){
		...
		foreach($data["seccIndi"]["relacion_id"] as $key => $relacID){
			...
			///->Facebook
			if($data["seccIndi"]["cat_seccion_tipo_id"][$key] == 7){
				$info["Concentrador"].= $this->facebook_lib->iniArmado($relacID,$data["seccIndi"]["cat_seccion_tipo_id"][$key]);
			}
			
			///->GTranslate
			if($data["seccIndi"]["cat_seccion_tipo_id"][$key] == 8){
				$info["Concentrador"].= $this->gtranslate_lib->iniArmado($relacID,$data["seccIndi"]["cat_seccion_tipo_id"][$key]);
			}
		}
